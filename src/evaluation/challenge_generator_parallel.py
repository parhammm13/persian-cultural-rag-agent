...
CHALLENGE_GENERATOR_VERSION = "3.0-parallel-full-corpus-multi"
...
def main() -> None:
    ...
    if args.max_multi_attempts < 1:
        parser.error("--max-multi-attempts must be at least 1")
    ...
    # Each worker owns its own ChallengeGenerator and cache directory.
    # This avoids concurrent writes to one challenge_cache.json file.
    worker_state = local()
    ...
    def get_generator() -> ChallengeGenerator:
        ...
        if generator is None:
            ...
            generator = ChallengeGenerator(
                cache_dir=out_dir / ".challenge_cache_parallel" / f"worker_{worker_number:02d}"
            )
    ...
    def run_single(
        ...
        try:
            return get_generator().generate_single(
                original=original,
                challenge_type=challenge_type,
                force=args.force,
            )
            ...
            print(
                f"[single error] type={challenge_type} "
                f"page={original.get('page_id', '')}: {exc}"
            )
            return None

    def run_multi(page: dict[str, Any]) -> dict[str, Any] | None:
        try:
            return get_generator().generate_multi(
                page=page,
                child_map=child_map,
                force=args.force,
            )
        except Exception as exc:
            print(f"[multi error] page={page.get('page_id', '')}: {exc}")
            return None

    # Read model name without creating a shared generator instance.
    try:
        _, _, configured_model = _get_openrouter_config()
    except Exception:
        configured_model = "unknown"

    print(f"[version] {CHALLENGE_GENERATOR_VERSION}")
    print(f"[llm] model={configured_model}")
    ...
    with ThreadPoolExecutor(max_workers=args.workers) as executor:
        # ------------------------------------------------------------
        # 1) Single-evidence hard questions
        # ------------------------------------------------------------
        # Generate per challenge type in bounded parallel batches.
        # Batching avoids sending dozens of unnecessary requests after a
        # challenge-type quota has already been reached.
        for type_index, (challenge_type, quota) in enumerate(
            SINGLE_CHALLENGE_QUOTAS.items()
        ):
            type_rng = random.Random(args.seed + 1000 + type_index)
            pool = list(originals)
            type_rng.shuffle(pool)

            cursor = 0
            attempts = 0

            while type_counts[challenge_type] < quota and cursor < len(pool):
                remaining = quota - type_counts[challenge_type]

                # Slight over-provisioning helps absorb rejects while keeping
                # unnecessary requests bounded.
                batch_size = min(
                    len(pool) - cursor,
                    max(args.workers, remaining),
                )
                batch = pool[cursor : cursor + batch_size]
                cursor += batch_size

                futures = [
                    executor.submit(run_single, original, challenge_type)
                    for original in batch
                ]

                # Consume in input order for deterministic acceptance order.
                for original, future in zip(batch, futures):
                    ...
                    if result is None:
                        print(
                            f"[single {challenge_type} {attempts}] "
                            f"page={original.get('page_id', '')} -> rejected"
                        )
                        continue
                    ...
                    if nq in seen_questions:
                        print(
                            f"[single {challenge_type} {attempts}] "
                            f"page={original.get('page_id', '')} -> duplicate"
                        )
                        continue
                    ...
                    print(
                        f"[single {challenge_type} {attempts}] "
                        f"page={original.get('page_id', '')} -> "
                        f"OK ({type_counts[challenge_type]}/{quota})"
                    )

        # ------------------------------------------------------------
        # 2) Genuine multi-evidence questions
        # ------------------------------------------------------------
        multi_pages = _candidate_multi_pages(page_map, rng)
        multi_pages = multi_pages[: args.max_multi_attempts]
        ...
        while (
            multi_count < args.multi_target
            and cursor < len(multi_pages)
            and multi_attempts < args.max_multi_attempts
        ):
            remaining = args.multi_target - multi_count

            # Multi generation is expensive (generation + validation), so keep
            # the speculative batch close to worker count / remaining target.
            batch_size = min(
                len(multi_pages) - cursor,
                args.max_multi_attempts - multi_attempts,
                max(args.workers, remaining),
            )
            batch = multi_pages[cursor : cursor + batch_size]
            cursor += batch_size

            futures = [
                executor.submit(run_multi, page)
                for page in batch
            ]
    ...
    if multi_count < args.multi_target:
        print(
            ...
            "Increase --max-multi-attempts or inspect the corpus for richer "
